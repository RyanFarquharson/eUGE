...
But that hides many stories.  What we actually need is usage - i.e. the increment between each read.
...
This was done by dividing each usage by the number of days in each increment.  Happily because we're using the date dat type, calculating the number of days was a cinch.
...
Ok. Now we have something meaningful to work with.  I did some more cleaning up by removing unnecessary intermediary columns and made a table called eUGE3 with months, years and seasons as factors.  Unfotunately, becaus I'm a newbie, I couldn't figure out a quick way to find the month which ends closest to each read date, so I just made some month, year and season lists and added them using mutate.
...
Some time ago, before it became cool, and then became a money making exercise for the wealthy, we installed a small (1 kW) photovoltaic system connected to the grid.  SO now first up we're going to look at how it is performing.
...
Note the seasonality.  As to be expected, we're generating and exporting much more elctricity in summer than in winter.  Note that the spread in the export data is greater than for the generation data.  Again, this makes sense because over the years, the amount of the generated electricity that we consume within the household has varied.
...
Here, PV efficiency is the amount of electricity that our teeny weeny 1 kW PV system generates per hour of operation.  In summer we're getting around 0.3 kW per hour.  Winter looks depressing, doesn't it?  Wondering what that dip in the middle of summer is?  High temperatures do lead to lower panel performance.  It would be interesting to get some temperature data and do some comparisons.  Another explanation is that the panels face north and are tilted to get better mid-year performance. We also have large trees east and west.  So during the long summer days when the sun rises in the south-east and sets and the south-west, there are many daylight hours when the inverter is working but there isn't any direct light on the panels.  Don't you love visualisation?
...
SO as you can see, there are many stories to be told.  

If was was limited to only 1 figure, this is what it woudl look like.
